...
if(isset($_POST['report_all'])) {
    ...
    forEach($dept_codes_ex as $deptcode => $deptname) {
        ...
        for($sem=1;$sem<=8;$sem++){
            ...
            if($tableBody != null && $tableHeader != null) {
                $template = $html_header.'<p class="bold" style="font-size:11px;">Branch Code:'.$deptcode.'-'.$dept_codes_ex[$deptcode].'</p>'.'<p class="bold"  style="font-size:11px;">Sem: '.str_pad($sem,2,"0",STR_PAD_LEFT).'</p>'.$tableHeader.$tableBody.$html_footer;
    ...
    $mpdf->Output('report_all_department.pdf','D');
    ob_clean();
    exit;
} else {
